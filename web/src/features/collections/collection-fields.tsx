...
import { Braces, Check, ChevronRight, Copy, KeyRound, Pencil, Plus, Save, Trash2, Undo2 } from 'lucide-react'
...
export function CollectionFields({ requirement, canEdit, onEditingChange, onSavingChange }: { requirement: CollectionDetail; canEdit: boolean; onEditingChange?: (editing: boolean) => void; onSavingChange?: (saving: boolean) => void }) {
  ...
  return <section className="requirement-fields" aria-label={t('fields.unified')}>
    <div className={`requirement-fields-toolbar ${draft ? 'is-editing' : ''}`}>
      <div className="requirement-fields-heading">
        ...
        <span className={`requirement-field-state ${draft || saved ? 'is-draft' : ''}`}>{t(draft || saved ? 'fields.draft' : 'fields.sourceSummary')}</span>
      ...
      <div className="requirement-field-actions">
        {canEdit && availableContracts.length > 0 && (
          <Button size="sm" variant="outline" disabled={mutation.isPending} onClick={() => setImportSourceOpen(true)}>
            <Copy />{t('fields.generateFromSource')}
          </Button>
        )}
        {canEdit && !draft && <Button ref={editTrigger} size="sm" variant="outline" onClick={start}><Pencil />{t('fields.edit')}</Button>}
        {draft && <>
          {removed && <Button size="sm" variant="ghost" disabled={mutation.isPending || draft.fields.some((field) => field.key === removed.value.key)} onClick={() => { const next = [...draft.fields]; next.splice(Math.min(removed.index, next.length), 0, removed.value); setDraft({ ...draft, fields: next }); setRemoved(null) }}><Undo2 />{t('fields.undoDelete')}</Button>}
          <Button ref={editTrigger} size="sm" variant="outline" disabled={mutation.isPending || draft.fields.length >= 100} onClick={event => { dialogTrigger.current = event.currentTarget; setFieldEdit({ value: blankField(), index: -1 }) }}><Plus />{t('fields.add')}</Button>
          ...
          <Button size="sm" disabled={mutation.isPending || !canEdit} onClick={() => mutation.mutate(draft)}><Save />{t(mutation.isPending ? 'collections.saving' : 'fields.saveDraft')}</Button>
        </>}
      </div>
    </div>
    {(saved || draft) && <p className="requirement-field-notice">{t('fields.compactDraftNotice')}</p>}
    {savedNotice && <p role="status" className="requirement-save-success"><Check size={14} />{t('fields.saved')}</p>}
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              {available.length === 1 ? (
                <div className="rounded border px-3 py-2 text-sm bg-muted/30">
                  ...
                  <span className="ml-2 text-muted-foreground">({selectedSource?.fields.length} 个字段)</span>
                </div>
              ) : (
                  ...
                  <SelectContent>
                    {available.map((c) => (
                      <SelectItem key={c.sourceId} value={c.sourceId}>
                        {c.sourceName} ({c.fields.length} 个字段)
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            {selectedSource && (
              <div className="rounded bg-muted/50 p-2.5 text-xs text-muted-foreground">
                包含字段：{selectedSource.fields.map((f) => f.label || f.key).join('、')}
              </div>
            )}
          </div>
        )}
