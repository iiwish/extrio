...
function CollectionContent({ requirement, reload, readError }: { requirement: CollectionDetail; reload: () => void; readError: Error | null }) {
  ...
  return <div className="page-frame collection-detail-page">
    <header className="collection-detail-header">
      <div className="requirement-heading"><div className="requirement-title-row"><span className={`requirement-lifecycle ${archived ? 'is-archived' : ''}`}>{archived ? <Archive size={13} /> : <Check size={13} />}{t(archived ? 'collections.archived' : 'collections.active')}</span><h1>{requirement.name}</h1></div><div className="requirement-header-meta"><span>{t('collections.sourcePublication')} <strong>{requirement.publishedSourceCount} / {requirement.sourceCount}</strong></span><span>{t('fields.updatedAt')} <time dateTime={requirement.updatedAt}>{new Date(requirement.updatedAt).toLocaleString(i18n.resolvedLanguage)}</time></span></div></div>
      {canEdit && <div className="collection-detail-actions">
        {archived ? <Button variant="outline" disabled={pending} onClick={() => { reset(); update.mutate({ revision: requirement.revision, status: 'active' }) }}><ArchiveRestore />{t('collections.restore')}</Button>
          : <><Button ref={editTrigger} variant="outline" size="sm" disabled={pending || fieldEditing} onClick={() => { reset(); setNotice(''); setEditing(requirement) }}><Pencil />{t('collections.edit')}</Button>
            <Button asChild size="sm"><Link to={workspaceLink(`/collectors/new?collection=${encodeURIComponent(requirement.id)}`)}><Plus />{t('collections.addSource')}</Link></Button>
        ...
        {requirement.sourceCount === 0 && <Button variant="ghost" size="icon-sm" title={t('collections.delete')} aria-label={t('collections.delete')} disabled={pending || fieldEditing} onClick={() => { reset(); setConfirmation({ type: 'delete', revision: requirement.revision }) }}><Trash2 /></Button>}
      </div>}
    </header>
    ...
    <Tabs ref={tabsRef} value={section} onValueChange={value => { const next = new URLSearchParams(params); if (value === 'fields') next.delete('section'); else next.set('section', value); setParams(next) }} className="requirement-detail-tabs">
      ...
      <TabsContent value="fields" forceMount><CollectionFields requirement={requirement} canEdit={canEdit && !archived} onEditingChange={setFieldEditing} onSavingChange={setFieldSaving} />{requirement.sourceCount === 0 && <p className="requirement-no-sources">{t('collections.noSources')}</p>}</TabsContent>
    ...
    <section aria-labelledby="collection-sources-title">
      ...
      {requirement.sources.length === 0 ? <div className="collections-empty"><Layers3 /><h2>{t('collections.noSources')}</h2></div>
        : <div className="collections-table-scroll"><table className="collection-sources-table">
          <thead><tr><th scope="col">{t('nav.collectors')}</th><th scope="col">{t('collections.sourceStatus')}</th><th scope="col">{t('fields.executionFields')}</th></tr></thead>
          <tbody>{requirement.sources.map((source) => { const contract = requirement.sourceContracts?.find((item) => item.sourceId === source.id); return <tr key={source.id}>
            ...
            <td><StatusBadge status={source.status} /></td><td>{!contract || contract.state === 'unavailable' ? <span className="requirement-field-diff">{t('fields.unknownCount')}</span> : contract.fields.length}</td>
          </tr> })}</tbody>
        </table></div>}
    </section>
      </TabsContent>
