...
it('previews actual output fields even before a requirement draft exists', () => {
  ...
  expect(screen.queryByRole('button', { name: '字段工具' })).not.toBeInTheDocument()
})
...
it('generates requirement fields from source with replace or merge mode', async () => {
  ...
  await user.click(screen.getByRole('menuitem', { name: '根据来源生成字段' }))
...
it('restores focus to field tools after closing source import without writing', async () => {
  const update = vi.spyOn(api, 'updateCollection')
  mount()
  const user = userEvent.setup()
  const tools = screen.getByRole('button', { name: '字段工具' })
...
it('opens a field template from the keyboard and restores focus after cancel', async () => {
  vi.spyOn(api, 'collectionTemplates').mockResolvedValue([])
  mount()
  const user = userEvent.setup()
  const tools = screen.getByRole('button', { name: '字段工具' })
  tools.focus()
  ...
  expect(dialog).toHaveTextContent('字段模板')
...
it('previews templates without writing and disables tools during local editing', async () => {
  ...
  const user = userEvent.setup()
  await user.click(screen.getByRole('button', { name: '字段工具' }))
  await user.click(screen.getByRole('menuitem', { name: '字段模板' }))
  ...
  await user.click(screen.getByRole('button', { name: '编辑字段' }))
  expect(screen.getByRole('button', { name: '字段工具' })).toBeDisabled()
})
