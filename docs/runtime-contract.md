...
本版本按[单组织适用合同](planning/v1.0-scope-matrix.md)和[自托管运维手册](self-hosted-operations.md)验收：数据库队列、同主机 Worker、匿名受限来源、Webhook、本地 sampled/metadata_only 证据及空实例离线恢复。下文的分布式 job envelope、Kafka、完整 replayable 分片、统计漂移基线及多层配额属于后续平台设计，不代表已交付能力。当前 job 默认租约 120 秒、按租约三分之一续期，最多三次领取；Webhook 当前每个重试周期最多五次尝试，使用稳定 Delivery ID 与独立租约；实际 Header 是 `X-Extrio-Signature`、`X-Extrio-Delivery-Id`、`Idempotency-Key`、`Content-Digest`。数据/Checkpoint/outbox/Operation 在有效 job 租约事务内完成，失败或取消不提升结果。SLO 与灾备目标保持目标性质，必须以相应运营证据验收。
...
来源删除原子关闭调度并保留不可恢复墓碑。队列提交与删除使用同一来源锁；实际活动任务、运行和未完成迁移阻止删除。调度领取后创建运行前再次验证来源存在且可执行，已删除来源不能通过旧引用启动任务。来源身份及全部历史外键保留，URL 可用于新身份；原有投递不因来源删除取消或重发。
...
来源 AI 编译执行采用 `adaptive-dom-v1` 只读协议：`expand_nodes` 分页展开快照目录，`read_nodes` 读取指定节点片段，`propose_rule` 提交现有发现或 RulePlan 对象。页面编号限定本次列表及最多三个详情快照，网页文字和属性一律作为不可信数据。片段携带 digest、范围、祖先和 nextCursor；生产字段值与验证取自完整快照。生成与修复共享 TaskBudget，最多 8 次调用、120000 输入 token、16000 输出 token、240 秒；恢复以首次执行时间和持久化预算约束，不重置额度。

模型能力从当前供应商/模型配置冻结，limits 可显式指定 contextTokens、maxInputTokens、maxOutputTokens、reasoningTokens；未配置时为 32768/32768/4096/0 并标记 conservative_default。每轮先扣除输出、推理预留和至少 256 token 或上下文 5% 的安全余量，再装入不可截断合同、页面目录、最近证据和结构化反馈。匹配 tokenizer 带额外封装余量，未知模型按 UTF-8 保守估算，供应商拒绝上下文时有界缩减证据包，不静默截掉合同。

调用前在拥有租约时持久化预留，缺失 usage 使用预留消耗；实际 usage 仅用于校准，不推算未配置的费用。候选须通过全部样本字段、标题、URL 交接和身份验证；反馈允许预算内补读重提案。取消、超时、失去租约或预算失败均不提交未验证候选，保留原规则。读取诊断不包含网页正文；readFragmentChars 为每节点范围合并后的片段字符总量，父子节点重叠内容可重复计算，不能当作整页独立字符覆盖率。
...
匿名来源 HTTPS 在 TLS 握手明确返回 `BAD_ECPOINT` 时，允许在同一次请求预算内使用 P-256 兼容握手重试一次；受信 CA、证书和主机名校验、TLS 最低版本、原始 Host/SNI 与固定目标 IP 均保持不变，不关闭校验或降级 HTTP。

受限浏览器拒绝站外脚本、样式、图片、字体和媒体请求并记录诊断，不因这些辅助资源缺失直接丢弃有效页面。入口、iframe、详情文档、XHR/fetch 数据请求的越界仍导致失败；任何辅助资源都不能扩展 allowedHosts，页面内容仍必须通过结构、字段与样本验证。
...
4. list/detail 的同名输出字段存在明确的 `fieldBindings[key]=list.key` 或 `detail.key` 时，字段定义、审核样本与结果值使用指定阶段，缺失值不跨阶段回填。无同名显式绑定时保留 detail 优先规则；冲突必须在编译期验证。
5. 所有字段完成类型转换和 transforms 后，再进行质量门、entityKey、outputContractDigest 和 payloadFingerprint 计算。

规则生成与修复分别构造列表和详情的模型证据：列表优先保留重复记录与链接，详情优先保留低链接密度正文区域及真实祖先结构，并为 head 标题/元数据和文档上下文预留预算。模型证据是有界样本，长文本节点可截断；原始页面 Artifact 与确定性字段验证使用完整抓取结果，不使用裁剪后的模型证据。候选生成完成不等于字段已通过审核，缺失字段仍进入原有审核门。
