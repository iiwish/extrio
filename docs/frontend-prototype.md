...
| `/collectors` | 采集来源 | 创建和继续一个 Collector 闭环 |
...
7. Collector 详情的采集来源定义与规则工作区在两种支持视口均可扫描和操作；需求归属与数据合同只读，名称修改保留活动规则状态，来源采集说明或入口修改使候选失效并提供保存后重新生成路径；直接规则编辑产生新候选 digest、清空旧审核决定并显示最近样本验证结果，已发布 RuleVersion 始终只读。
