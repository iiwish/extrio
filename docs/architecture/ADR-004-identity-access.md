...
Extrio v0.2 采用外部 IdP + OIDC/OAuth 2.0 授权码和 PKCE 认证人类用户，采用短期服务端会话访问 Web 控制面；服务间通信采用独立工作负载身份和 mTLS。平台不保存本地用户密码，不把人类 session token 用作服务身份。
...
| 方案 | 结论 | 原因 |
| --- | --- | --- |
| 本地密码体系 | 不采用 | 增加密码存储、恢复和 MFA 责任，不适合 v0.2 |
