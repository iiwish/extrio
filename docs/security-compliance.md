...
单组织自托管安全边界以[1.0 适用合同](planning/v1.0-scope-matrix.md)和[运维手册](self-hosted-operations.md)为准：本地 administrator/engineer/reviewer/viewer、可撤销会话、服务端 RBAC、受限匿名网络、受保护的本地签名/加密 key、不可变规则与审计、离线一致备份恢复。首次与最后管理员保护、密码重置和并发登录的会话创建在数据库内协调；已存在密文/注册公钥时丢失 key 必须失败关闭。实际发布 actor 进入证明；同一人可承担批准与审核，不冒充四眼策略。MCP token 是独立的受信任工具访问，不继承浏览器用户角色。当前运行适配器拒绝 AccessProfile 凭据注入、POST、静态认证 Header 和访问控制绕过。

下文的多租户隔离、OIDC/MFA、四眼审批、凭据代理、Kafka、Secret Manager、只持公钥的分布式 Worker、对象存储与集群网络策略是后续平台设计合同，不是本机共享卷部署已经提供的安全隔离。生产仍需运维负责 TLS、系统访问、卷和备份加密、最小数据库账号及密钥事故响应；有限本地测试不证明这些外部控制已配置。
