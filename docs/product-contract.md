...
规则生成和修复使用 DOM/JSON 结构索引、模型按需补读与完整样本验证。模型只能读取已获取快照中的节点和合法游标，没有网络、文件或代码执行权限。超长目录与节点分页返回，局部证据不替代完整快照提取。输出合同、列表与详情绑定、字段身份和质量门保持固定；缺失字段、标题不一致、详情 URL 或实体冲突反馈后允许在剩余预算内重提案，不自动接受风险或排除字段。

每次新执行及其恢复共享 16 次调用、120000 输入 token、16000 输出 token、240 秒上限；历史恢复不提高原调用上限。发现阶段保留 4 次给后续编译与校验纠错。每轮最多 6 个读取请求，按上下文余量分配每节点最多 6000 字符或 16 个子节点；连续 3 次无新证据或新提案则停止。校验反馈包含具体字段路径和原因。调用前预留，实际 usage 校准，缺失 usage 按预留计入。预算不足、取消和失去租约不提交未验证候选，不改变原规则和生产数据。管理员可配置上下文窗口、输入/输出上限和推理预留；旧模型配置沿用保守默认值。token 计数方式与限额来源可见，不根据型号名称宣称代理接口能力。

AI 任务显示读取阶段、预算、各页面索引与读取节点、片段字符数、完整样本验证状态及停止原因。覆盖数字仅描述读取，不代表准确率；诊断不复制网页正文、提示词、模型响应或凭据。窗口档位模拟测试与真实模型验收分别记证，未实测模型不宣称兼容。候选仍需人工审核发布，生产 Run 不调用模型。

## 核心操作上下文

来源详情的已受理任务默认打开执行日志弹窗，集中展示实际进度、阶段时间、终态、错误原因与任务详情入口。收起不停止执行，后续轮询不重新打开；页头保留带状态的查看按钮，新任务默认打开，最近 AI 日志可在刷新后按需查看。明确的浏览器导航超时在原有网络预算内最多自动重试一次，不放宽主机、robots 或安全边界；持续失败提供可理解说明与关联诊断，不报告虚构的 HTTP 状态。
...
平台结合 Source、CollectionVersion 和 CollectorOverride 生成 RuleVersionDraft。每个 Collector 通过稳定 `collectionId` 与 `collectionName` 归属一个业务采集需求；工作区同时显示只读需求归属、可编辑来源采集说明、Source 入口、活动 RuleVersion 与候选规则摘要。CollectionEditor 可以修改 Collector 名称；修改来源采集说明或 Source 入口会关闭调度，清除候选、活动规则与 Checkpoint，后续运行需重新探索、审核发布。探索通过 Crawl4AI 固定网页证据，当前默认模型先理解入口拓扑并编译列表发现计划，再结合详情样本编译 `RulePlan`；平台只接受受约束的 CSS/JSONPath、字段、分页、身份和指纹声明，并把通过 Schema、安全与样本验证的结果转换为 GatherSpec。规则编辑表单按执行顺序呈现列表 Item selector、网页业务字段 selector、分页、`detailUrl` 交接和详情字段 selector，不展示系统管理字段，也不重复展示不可编辑的请求配置、字段语义与输出合同；CollectionEditor 可以直接编辑两个阶段的 selector 与分页参数；系统把编辑记录为受控 Override，使用最近探索样本重新验证并生成新的候选 digest。字段语义服从 CollectionVersion。RuleReviewer 查看字段预览、失败样本、网络边界和编译谱系后发布规则；任何编辑都不能改写已发布 RuleVersion。
