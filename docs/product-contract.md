...
| `FR-011` | 平台必须支持 Kafka 与 Webhook Sink，并为每个 Item 事件和 SinkVersion 建立可重试、可审计的 Delivery；同目标人工重新交付不得创建重复逻辑 Delivery。 |
...
| `NFR-013` | 人类身份必须通过 OIDC/OAuth 2.0 授权码 + PKCE 接入；高风险操作支持 MFA step-up；服务身份必须与人类会话分离。 |
...
- 身份认证：人类用户通过 OIDC/OAuth 2.0 授权码 + PKCE；服务通过工作负载身份；v0.2 不保存本地密码。
