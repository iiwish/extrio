...
该闭环是可运行的开发基线，不等于 v0.2 生产发布。生产验收仍要求 PostgreSQL、Redis Streams、对象存储、OIDC/RBAC、KMS/HSM 托管签名、集中审计与导出、分布式高可用调度、交付、完整 SSRF/DNS rebinding 防护、租户隔离、重试栅栏、容量与灾备证据。SQLite、API 进程内调度扫描、文件型开发私钥、显式风险接受的匿名 HTTP 和本地单 Worker 只允许用于开发与验收环境。
