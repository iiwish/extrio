...
def test_public_demo_keeps_runtime_authenticated_and_private():
    ...
    assert manifest["ingress"]["hostname"] == "app.extrio.ouvo.ai"
    ...
    for service in ("api", "worker"):
        ...
        assert "https://app.extrio.ouvo.ai" in config["environment"]["EXTRIO_CORS_ORIGINS"].split(",")
...
def test_public_proxy_requires_https_and_blocks_bootstrap():
    ...
    assert "server_name app.extrio.ouvo.ai;" in config
    assert "return 308 https://app.extrio.ouvo.ai$request_uri;" in config
